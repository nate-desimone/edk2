...
--*/
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (Index == 1) {
        //
        // Assume the second FD contains the unit tests
        // Tell PEI Core about its existence so unit test PEIMs will be dispatched
        //
        PeiServicesInstallFvInfoPpi (NULL, (VOID *) (UINTN) FdBase, (UINT32) FvHeader->FvLength, NULL, NULL);
      }
    }
